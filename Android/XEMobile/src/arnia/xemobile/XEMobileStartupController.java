...
public class XEMobileStartupController extends XEActivity {
	...
	protected void onResume() {		
		...
		Cursor cursor = db.rawQuery("select count(*) totalWebsite from " + dbHelper.XE_SITES, null);
		cursor.moveToFirst();
		int recordCount = cursor.getInt(0);
		cursor.close();
		db.close();		
		if(recordCount>0){
			Intent callDashboard = new Intent(this,XEMobileMainActivityController.class);
			startActivity(callDashboard);
			
		}else{
			Intent callAddNewSite = new Intent(this,XEMobileWelcomeScreenController.class);
